...
void MatrixVectorELLPACK(int M, int N, int NNZ, int MAXNZ, const int* JA,
 const double* AZ, const double* x, double* restrict y);
double check_result(int M, double* restrict y_s_c, double* restrict y);
void save_result_omp(char *program_name, char* matrix_file, int M, int N,
                 ...
                 double time_ell_1d_omp,  double mflops_ell_1d_omp,  double max_diff_ell_1d_omp,
                 double time_ell_2d_omp,  double mflops_ell_2d_omp,  double max_diff_ell_2d_omp,
                 double time_ell_2dt_omp, double mflops_ell_2dt_omp, double max_diff_ell_2dt_omp);
...
int main(int argc, char** argv) 
{
  ...
  double max_diff_ell_serial = check_result(matrix_csr.M, y_s_c, y_s_e);
  ...
  double max_diff_csr_omp = check_result(matrix_csr.M, y_s_c, y_o_c);
...
#pragma omp master
{
  ...
  double max_diff_ell_1d_omp = check_result(matrix_csr.M, y_s_c, y_o_e1d);

#pragma omp parallel 
{
#pragma omp master
{
  fprintf(stdout," [ELL 1D OMP] with %d thread chunk_size %d: time %lf  MFLOPS %lf max_diff %lf\n",
	  omp_get_num_threads(), chunk_size, time_ell_1d_omp, mflops_ell_1d_omp, max_diff_ell_1d_omp);
}
}
  ...
  double max_diff_ell_2d_omp = check_result(matrix_csr.M, y_s_c, y_o_e2d);

#pragma omp parallel 
{
#pragma omp master
{
  fprintf(stdout," [ELL 2D OMP] with %d thread chunk_size %d: time %lf  MFLOPS %lf max_diff %lf\n",
	  omp_get_num_threads(), chunk_size, time_ell_2d_omp, mflops_ell_2d_omp, max_diff_ell_2d_omp);
}
}
  ...
  double max_diff_ell_2dt_omp = check_result(matrix_csr.M, y_s_c, y_o_e2dt);

#pragma omp parallel 
{
#pragma omp master
{
  fprintf(stdout," [ELL 2DT OMP] with %d thread chunk_size %d: time %lf  MFLOPS %lf max_diff %lf\n",
	  omp_get_num_threads(), chunk_size, time_ell_2dt_omp, mflops_ell_2dt_omp, max_diff_ell_2dt_omp);
}
}
                 ...
                 double time_ell_2dt_omp, double mflops_ell_2dt_omp, double max_diff_ell_2dt_omp)
{
  ...
  FILE *fp;
  char filename[] = "result_omp_v2.csv";  //file name
  fp = fopen(filename, "a+");
  if (fp == NULL) {
...
fprintf(fp, "time_csr_omp,mflops_csr_omp,max_diff_csr_omp,");
fprintf(fp, "time_ell_d1_omp,mflops_ell_d1_omp,max_diff_ell_d1_omp,");
fprintf(fp, "time_ell_2d_omp,mflops_ell_2d_omp,max_diff_ell_2d_omp,");
fprintf(fp, "time_ell_2dt_omp,mflops_ell_2dt_omp,max_diff_ell_2dt_omp\n");
  }
