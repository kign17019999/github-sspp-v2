...
int blocks_per_mp = 0; // Sufficiently large for memory transaction hiding
int threads_per_mp=0;
...
void reduce_alloc_wrk()
{
  ...
  if (max_blocks == 0) {
    ...
    threads_per_mp = prop->maxThreadsPerMultiProcessor;
    blocks_per_mp = 4*((threads_per_mp+thread_block-1)/thread_block);
